    ...
    return (
        <div className="animate-in p-0 fade-in duration-200 z-10 relative">
            ...
            <div className="p-6 relative">
                ...
                {activeTab === 'spine' && TOPICS.map((ts, idx) => {
                    const { isDone, isActive } = checkStatus(ts.id, idx, state.tIdx);
                    return <div key={ts.id}>{renderRow(isDone, isActive, `Module ${idx + 1}`, ts.text, idx === TOPICS.length - 1)}</div>;
                })}
                ...
                {activeTab === 'qs' && (
                    <>
                        ...
                        {QUESTIONS.slice(Math.max(0, state.qIdx - 10), Math.min(QUESTIONS.length, state.qIdx + 110)).map((q, idx) => {
                            ...
                            return <div key={q.id}>{renderRow(isDone, isActive, `Question ${actualIdx + 1} // 10 XP`, q.text, actualIdx === QUESTIONS.length - 1)}</div>;
                        })}
                    </>
                )}
                ...
                {activeTab === 'skills' && (
                    <>
                        ...
                        {BASIC_SKILLS.filter(s => s.id.startsWith('bs_')).map((s: any, idx: number) => (
                        ...
                        {BASIC_SKILLS.filter(s => s.id.startsWith('ps_')).map((s: any, idx: number) => {
                            const pSkills = BASIC_SKILLS.filter(sk => sk.id.startsWith('ps_'));
                            return (
                                <div key={s.id} className="relative py-4 pl-6 flex gap-4 items-start">
                                    <div className={`absolute left-[5px] top-0 w-[2px] bg-[var(--surface-alt)] z-1 ${idx === pSkills.length - 1 ? 'bottom-[50%] bg-gradient-to-b from-[var(--surface-alt)] to-transparent' : 'bottom-0'}`} />
                                    <div className="text-[0.85rem] text-[var(--text-muted)]">#{idx + 1} {s.name}</div>
                                </div>
                            );
                        })}
