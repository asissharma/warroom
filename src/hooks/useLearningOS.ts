...
export function useLearningOS() {
    ...
    const getMasterItem = useCallback((id: string) => {
        ...
        if (id.startsWith('ts_')) return TECH_SPINE.find((x: any) => x.id === id);
        if (id.startsWith('bs_')) return BASIC_SKILLS.find((x: any) => x.id === id);
        if (id.startsWith('ps_')) return PAYABLE_SKILLS.find((x: any) => x.id === id);
        return null;
    }, []);
    ...
    const checkAndPopulateQueue = useCallback((currentState: AppState) => {
        ...
        // 4. Tech spine
        while (counts.tech_spine < 1 && s.tIdx < (TECH_SPINE?.length || 0)) {
            let ts: any = TECH_SPINE[s.tIdx];
            s.activeTasks.push({ id: ts.id, type: ts.type, title: ts.focusArea, desc: ts.text || `Module ${s.tIdx + 1}`, done: false });
            s.tIdx++; counts.tech_spine++; updated = true;
        }

        // 5. Basic Skills
        while (counts.basic_skill < 1 && s.sIdx < (BASIC_SKILLS?.length || 0)) {
            let bs: any = BASIC_SKILLS[s.sIdx];
            s.activeTasks.push({ id: bs.id, type: bs.type, title: bs.name, desc: 'Cognitive Foundation', done: false });
            s.sIdx++; counts.basic_skill++; updated = true;
        }

        // 6. Payable
        while (counts.payable_skill < 1 && s.payIdx < (PAYABLE_SKILLS?.length || 0)) {
            let ps: any = PAYABLE_SKILLS[s.payIdx];
            s.activeTasks.push({ id: ps.id, type: ps.type, title: ps.name, desc: 'High-Leverage Skill', done: false });
